...
The action will automatically include any files or directories based on a `include-paths` input. This input accepts a list of paths (separated by whitespace) that should be included in the sitemap, even if they would otherwise be excluded. This should be used with caution to include files that are not present in the workflow (e.g., files that are generated by a build step). For example, if you wish to include the directory `/include-these` as well as the individual file `/sitemap-include.html`, you can use the following:

```yml
    - name: Generate the sitemap
      uses: cicirello/generate-sitemap@v1
      with:
        include-paths: /include-these /sitemap-include.html
```

If you have many such cases to include, your workflow may be easier to
read if you use a YAML multi-line string, with the following:
